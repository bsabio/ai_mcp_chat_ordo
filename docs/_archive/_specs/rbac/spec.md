...
> **Status:** Implemented (v2.3 — all 6 sprints complete, QA-verified)  
...
POST /api/auth/register
  ...
  → 409: { error: string }    — "Email already registered" for duplicates;
  → 400: { error: string }    — validation message listing all invalid fields
