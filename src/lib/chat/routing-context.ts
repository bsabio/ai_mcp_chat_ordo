...
function getRoutingInstruction(snapshot: ConversationRoutingSnapshot): string {
  switch (snapshot.lane) {
    ...
    case "uncertain":
    default:
      return "Ask one brief clarifying question to determine whether the need is a customer workflow, technical implementation, or training outcome before making a lane-specific recommendation.";
  }
}
