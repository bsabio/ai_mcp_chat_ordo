...
export const EVAL_SCENARIOS: readonly EvalScenario[] = [
  ...
  },
  {
    ...
    expectedCheckpoints: [
      { id: "suggestions-suppressed", label: "missing or malformed suggestions do not synthesize follow-up chips", required: true },
  ...
  {
    id: "integrity-session-resolution-deterministic",
    name: "Integrity session resolution deterministic",
    cohortId: "runtime-integrity-auditor",
    layer: "deterministic",
    targetEnvironment: "ci",
    requiredFixtures: ["actionable assistant output", "closed assistant output"],
    expectedCheckpoints: [
      { id: "resolution-advanced", label: "actionable next steps trigger an advanced resolution kind", required: true },
      { id: "resolution-resolved", label: "explicit closed response state triggers a resolved kind", required: true },
    ],
    expectedToolBehaviors: [],
    scoreDimensions: ["customer_clarity", "safety"],
  },
  {
    id: "integrity-retrieval-quality-deterministic",
    name: "Integrity retrieval quality deterministic",
    cohortId: "runtime-integrity-auditor",
    layer: "deterministic",
    targetEnvironment: "ci",
    requiredFixtures: ["search corpus outputs"],
    expectedCheckpoints: [
      { id: "quality-strong", label: "search correctly produces strong quality for exact matches", required: true },
      { id: "quality-partial", label: "search correctly produces partial quality for weak matches", required: true },
      { id: "quality-none", label: "search correctly produces none quality for zero or extremely faint matches", required: true },
    ],
    expectedToolBehaviors: [],
    scoreDimensions: ["customer_clarity", "safety"],
  },
  {
    id: "integrity-two-stage-retrieval-deterministic",
    name: "Integrity two-stage retrieval deterministic",
    cohortId: "runtime-integrity-auditor",
    layer: "deterministic",
    targetEnvironment: "ci",
    requiredFixtures: ["strong corpus hit", "competing partial corpus hit", "two-stage retrieval contract"],
    expectedCheckpoints: [
      { id: "strong-auto-read", label: "strong matches auto-read the top section", required: true },
      { id: "partial-stays-locate-step", label: "partial or competing matches stay at the locate step", required: true },
      { id: "strong-prefetch-canonical", label: "prefetched sections keep canonical citation targets", required: true },
    ],
    expectedToolBehaviors: [],
    scoreDimensions: ["tool_correctness", "customer_clarity", "safety"],
  },
  {
    id: "integrity-structured-get-section-deterministic",
    name: "Integrity structured get section deterministic",
    cohortId: "runtime-integrity-auditor",
    layer: "deterministic",
    targetEnvironment: "ci",
    requiredFixtures: ["structured get_section payload", "related sections", "slug integrity fixture"],
    expectedCheckpoints: [
      { id: "structured-metadata-returned", label: "get_section returns structured metadata instead of flat markdown", required: true },
      { id: "related-sections-returned", label: "get_section returns adjacent related sections", required: true },
      { id: "citation-targets-safe", label: "related and fallback citation targets never dead-link", required: true },
    ],
    expectedToolBehaviors: [],
    scoreDimensions: ["tool_correctness", "customer_clarity", "safety"],
  },
...
export function validateEvalCatalog(): string[] {
  ...
  const requiredScenarioIds = [
    ...
    "integrity-session-resolution-deterministic",
    "integrity-retrieval-quality-deterministic",
    "organization-buyer-deterministic",
